...
figure_save_dir = 'flexgen-model-figures/flexgen-mode-opt13b-bs-{}-{}'
...
info_file = os.path.join(figure_save_dir, 'opt-13b-model-offload-bs-{}-{}-info.txt'.format(BS, FS))
gpu_util_save_path = os.path.join(
    figure_save_dir,
    'opt-13b-model-offload-bs-{}-{}-gpu_util.pdf'.format(BS, FS))
bitesize_save_prefix = os.path.join(figure_save_dir,
                                    'opt-13b-model-offload-bs-{}-{}-bitesize-'.format(BS, FS))
sector_access_count_prefix = os.path.join(figure_save_dir, 'opt-13b-model-offload-bs-{}-{}-sector-access-count-'.format(BS, FS))
sector_access_cdf_prefix = os.path.join(figure_save_dir, 'opt-13b-model-offload-bs-{}-{}-sector-access-cdf-'.format(BS, FS))
size_hist_op_prefix = os.path.join(figure_save_dir, 'opt-13b-model-offload-bs-{}-{}-size-hist-'.format(BS, FS))
...
if True:
    ...
    datalabel_size = 26
    datalabel_va = 'bottom'
    axis_tick_font_size = 65
    axis_label_font_size = 65
    legend_font_size = 65
...
if True:
    ...
    # Ensure both lines start at the same time range
    ...
    if legend_label != None:
        plt.legend(fontsize=legend_font_size,  prop={'size': 44}, labelspacing=0.1, loc='lower right')

    ax.set_xlim(0, 1500)
    ax.set_xticks(range(0, 1800, 300))
    ax.set_xticklabels([str(size // 60) for size in range(0, 1800, 300)])
    ax.set_ylim(0, 3)
...
if True:
    ...
    if False: #os.path.exists(sector_access_count_prefix + 'all.pkl'):
        ...
        for cur_ts, cur_op_inner, cur_start_sector, cur_num_sector in zip(timestamp, op, sector,
                                                    num_sectors):
            # x.extend([cur_ts] * cur_num_sector)
            # y.extend(range(cur_sector, cur_sector + cur_num_sector))
